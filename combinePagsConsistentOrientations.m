function [combinedPag, all] = combinePagsConsistentOrientations(inputpags, screen)
...
combinedPag.graph = ones*(~~max(all.pags, [], 3));
[Xs, Ys] = find(combinedPag.graph);
for iX =1:length(Xs)   
    Y = Ys(iX);
    X =Xs(iX);
    bothObserved = ~sum(all.isLatent([X, Y], :));
   % fprintf('processing %d-%d\n',X, Y)
    if ~any(~(all.pags(X, Y, bothObserved)==3))
        combinedPag.graph(X, Y)=3;
    elseif  ~any(~(all.pags(X, Y, bothObserved)==2))
        combinedPag.graph(X, Y)= 2;
    else combinedPag.graph(X, Y)=1;
    end
end
